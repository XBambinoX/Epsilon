...
public static class ImproperIntegrator
{
    ...
    public static double IntegrateImproper(
        this Expr expr,
        double a,
        double b,
        double tolerance = DefaultTolerance)
    ...
    private static double ComputeImproperIntegral(
        Expr expr,
        double a,
        double b,
        double tolerance)
    {
        if (a > b)
            return -ComputeImproperIntegral(expr, b, a, tolerance);

        bool aInfinite = double.IsNegativeInfinity(a);
        bool bInfinite = double.IsPositiveInfinity(b);

        if (!aInfinite && !bInfinite)
            return ComputeFiniteImproperIntegral(expr, a, b, tolerance);

        if (aInfinite && bInfinite)
        {
            const double split = 0.0;

            return ComputeImproperIntegral(expr, double.NegativeInfinity, split, tolerance)
                 + ComputeImproperIntegral(expr, split, double.PositiveInfinity, tolerance);
        }

        if (aInfinite)
            return ComputeLeftTail(expr, b, tolerance);

        return ComputeRightTail(expr, a, tolerance);
    }

    private static double ComputeFiniteImproperIntegral(
        Expr expr,
        double a,
        double b,
        double tolerance)
    {
        ...
        foreach (double singularity in singularities)
        {
            result += ComputeEndpointIntegral(
                expr,
                previous,
                singularity,
    ...
    private static double ComputeEndpointIntegral(
        Expr expr,
        double a,
        double b,
        bool rightSingularity,
        double tolerance)
    {
        const double epsilon = 1e-8;

        if (rightSingularity)
            return expr.Integrate(a, b - epsilon);

        if (a == b)
            return 0;

        return expr.Integrate(a + epsilon, b);
    }

    private static double ComputeRightTail(
        Expr expr,
        double start,
        double tolerance)
    {
        double previous = start + 10.0;
        double result = expr.Integrate(start, previous);

        for (int i = 0; i < 20; i++)
        {
            double next = start + Math.Pow(2.0, i + 4) * 10.0;

            double part = expr.Integrate(previous, next);
    ...
    private static double ComputeLeftTail(
        Expr expr,
        double end,
        double tolerance)
    {
        double previous = end - 10.0;
        double result = expr.Integrate(previous, end);

        for (int i = 0; i < 20; i++)
        {
            double next = end - Math.Pow(2.0, i + 4) * 10.0;

            double part = expr.Integrate(next, previous);
